	...
	public function render() {
		...
		<div id="<?php echo esc_attr( $this->id ); ?>_wrapper" class="ss-field-wrapper ss-field-spacing ss-clearfix"<?php SocialSnap_Fields::dependency_builder( $this->dependency ); ?>>
			...
			<div class="ss-field-element ss-clearfix">
				<textarea <?php echo esc_html( $this->placeholder ); ?> name="<?php echo esc_attr( $this->id ); ?>" id="<?php echo esc_attr( $this->id ); ?>" rows="<?php echo esc_attr( $this->rows ); ?>"><?php echo wp_kses( $this->value, socialsnap_get_allowed_html_tags( 'post' ) ); ?></textarea>
			</div>
		</div>
		<?php
		return ob_get_clean();
