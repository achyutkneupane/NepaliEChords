		...
		<div id="<?php echo esc_attr( $this->id ); ?>_wrapper" class="ss-field-wrapper ss-field-spacing ss-clearfix<?php echo esc_attr( $is_pro_class ); ?>"<?php SocialSnap_Fields::dependency_builder( $this->dependency ); ?>>
			...
			<?php if ( $is_pro ) { ?>
				...
				<div class="ss-field-element ss-clearfix">
					<input type="<?php echo esc_attr( $this->value_type ); ?>" <?php echo esc_html( $this->placeholder ); ?> name="<?php echo esc_attr( $this->id ); ?>" id="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $this->value ); ?>" <?php echo esc_html( $additional_params ); ?><?php echo esc_html( $this->readonly ); ?> />
				</div>
			<?php } ?>
		</div>
		<?php
